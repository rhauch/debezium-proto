...
final class DbzDatabases extends Service {
    ...
    protected void onStart(DbzNode node) {
        // Add a single-threaded consumer that will read the "schema-updates" topic.
        // We use a unique group ID so that we get all of the messages on this topic (not just our own) ...
        int numThreads = 1;
    ...
    DatabaseConnection connect(ExecutionContext context, DatabaseId dbId, long timeout, TimeUnit unit) {
        whenRunning(node -> {
            ...
            if (db == null) {
                db = handlers.requestAndWait(context, 1, timeout, unit, submitReadSchema(context, dbId, node),
                                             this::updateDatabase, notAvailable(dbId)).get();
            }
            assert db != null;
    ...
    private Consumer<RequestId> submitReadSchema(ExecutionContext context, DatabaseId id, DbzNode node) {
        return (requestId) -> {
            ...
            node.send(Topic.SCHEMA_PATCHES, context.databaseId().asString(), request);
        };
    }
    
    private ActiveDatabase updateDatabase(Document schemaReadReponse) {
