...
import org.debezium.api.doc.Path;
...
public final class Patch<IdType extends Identifier> implements Iterable<Patch.Operation> {
     ...
     */
    public static interface Operation extends BiConsumer<Document,Consumer<Path>> {
        /**
    ...
    protected static final class AddOp implements Add {
        ...
        @Override
        public void accept(Document doc, Consumer<Path> invalid ) {
            doc.set(Path.parse(this.path), true, value(),invalid);
        }
    }
        ...
        public void accept(Document doc, Consumer<Path> invalid ) {
            ...
            if ( result.isPresent() ) {
                ...
                if ( parent.isArray() ) {
                    ...
                    if ( index.isPresent() ) parent.asArray().remove(index.get().intValue());
                }
            }
            // Otherwise does not exist, so removal doesn't mean anything
        }
    }
    ...
    protected static final class ReplaceOp implements Replace {
        ...
        public void accept(Document doc, Consumer<Path> invalid ) {
            Path path = Path.parse(this.path);
            doc.set(path, true, value(), invalid);
        }
    }
    ...
    protected static final class CopyOp implements Copy {
        ...
        public void accept(Document doc, Consumer<Path> invalid ) {
            ...
            if ( from.isPresent() ) {
                ...
                doc.set(toPath, true, from.get(), invalid);
            }
        }
    }
            ...
            if ( fromParentPath.isPresent() ) {
                ...
                if ( fromParent.isPresent() ) {
                    ...
                    if ( parent.isDocument() ) {
                        ...
                        Path toPath = Path.parse(this.toPath);
                        doc.set(toPath, true, valueToMove, invalid);
                    }
                }
            }
        }
    ...
    protected static final class IncrementOp implements Increment {
        ...
        public void accept(Document doc, Consumer<Path> invalid ) {
            ...
            Value incremented = Value.create(MathOps.add(existing.get().asNumber(),value()));
    ...
    public Document apply(Document document, Consumer<Operation> failed ) {
        ops.stream().forEach((op) -> op.accept(document,(invalidPath)->{failed.accept(op);}));
        return document;
    }
}
