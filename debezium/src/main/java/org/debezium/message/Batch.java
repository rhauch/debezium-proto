...
public final class Batch<IdType extends Identifier> implements Iterable<Patch<IdType>> {
    ...
    public static interface Builder<IdType extends Identifier> {
        ...
        /**
         * Begin creating a new target object with the given identifier. When the editing is {@link Patch.Editor#end() completed},
         * the changes will be recorded
    ...
    protected static final class BatchBuilder<T extends Identifier> implements Builder<T> {
        ...
        protected final class PatchEditor implements Patch.Editor<Builder<T>> {
            ...
            @Override
            public Optional<Builder<T>> endIfChanged(Predicate<Operation> significant) {
                Optional<Patch<T>> patch = patchEditor.endIfChanged(significant);
                patch.ifPresent((p) -> {
                    patches.add(p);
                    patchEditor = null;
                });
